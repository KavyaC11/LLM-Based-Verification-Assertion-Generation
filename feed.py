"""
RTL Assertion Generator — Self-Refinement pipeline orchestrator
===============================================================
...
from phase7_memory      import check_staleness
from eval               import evaluate_rules_isolated
...
def _make_run_dirs(design_name: str, session_id: str) -> dict:
    ...
    root = Path("results") / design_name / f"{ts}_{session_id}_self_refine"
...
def process_rule(rule: dict, grounding_result: dict, rtl_ir: dict, spec_ir: dict, design_hash: str) -> dict:
    ...
    # Use Phase 10 Self-Refinement instead of standard generation
    ...
    # Note: Phase 10 natively handles update_memory internally via persist_feedback_signals
    return {
        "rule_id":    rule["id"],
        "rule":       rule,
        "assertion":  assertion,
...
def run_pipeline(design_name: str, max_rules: int = 5) -> list:
    ...
    _print_header(f"RTL Assertion Generator (Self-Refine) — {design_name}")
    ...
    print(f"\n{Fore.CYAN}Processing {len(rules_to_process)} rule(s) with Self-Refinement…{Style.RESET_ALL}")
    ...
    for i, rule in enumerate(rules_to_process, start=1):
        ...
        try:
            result = process_rule(rule, grounding_result, rtl_ir, spec_ir, sess["design_hash"])
    ...
    # Run formal evaluation with Yosys
    print(f"\n{Fore.CYAN}═══ Formal Evaluation (SymbiYosys) ═══{Style.RESET_ALL}")
    try:
        evaluate_rules_isolated(str(dirs["root"]), top_module=cfg.get("top_module", "eth_top"))
    except Exception as e:
        print(f"{Fore.RED}Formal evaluation failed to run: {e}{Style.RESET_ALL}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python feed.py <design_name> [max_rules]")
